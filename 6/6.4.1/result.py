...
async def run_demo(file_name):
    ...
    await storage.send_file(file_name)
    logger.info(f"File {file_name} sent")

    files = await storage.list_files()
    logger.info(f"Files in bucket: {files}")

    exists = await storage.file_exists(file_name)
    logger.info(f"File {file_name} exists: {exists}")

    await storage.fetch_file(file_name, "copy_" + file_name)
    logger.info(f"File {file_name} fetched")

    await storage.remove_file(file_name)
    logger.info(f"File {file_name} removed")
...
if __name__ == "__main__":
    file_name = "task.md"
    asyncio.run(run_demo(file_name))
