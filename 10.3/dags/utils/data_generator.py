...
def generate_all_data(base_dir="data"):
    """Генерировать все данные в указанную директорию"""
    # Создание директорий
    os.makedirs(f"{base_dir}/stores", exist_ok=True)
    os.makedirs(f"{base_dir}/products", exist_ok=True)
    os.makedirs(f"{base_dir}/customers", exist_ok=True)
    os.makedirs(f"{base_dir}/purchases", exist_ok=True)
    ...
    for network, count in store_networks:
        for i in range(count):
            ...
            stores.append(store)
            with open(f"{base_dir}/stores/{store_id}.json", "w", encoding="utf-8") as f:
    ...
    for i in range(20):
        ...
        with open(
            f"{base_dir}/products/{product['id']}.json", "w", encoding="utf-8"
        ) as f:
    ...
    for store in stores:
        ...
        with open(
            f"{base_dir}/customers/{customer_id}.json", "w", encoding="utf-8"
        ) as f:
    ...
    for i in range(200):
        ...
        with open(
            f"{base_dir}/purchases/{purchase['purchase_id']}.json",
            "w",
            encoding="utf-8",
        ) as f:
...
# Если скрипт запускается напрямую, используем текущую директорию
